...
- 동아리 약 60명(동시 접속 최대 100명까지 검증)
- 기본 팀: 매크로 분석 / 차트 분석 / 퀀트 투자 / 채권 · 원자재 (각 정원 15명, 설정으로 변경 가능)
...
동명이인 때문에 실제 참가자가 막히는 사고를 피하기 위해 **학번(10자리)**를 함께 받는다.
...
| 경로 | 설명 |
|---|---|
| `/` | 팀 카드 목록(잔여 n/정원, Realtime 갱신, 마감 배지) → 이름·학번 입력 → 확인 모달 → 제출. 오픈 전에는 서버 시각 기준 카운트다운. 신청 후 본인 배정 결과 화면으로 전환 |
...
1. 기존 DB 라면 `supabase/update-teams-2026.sql` 실행 후 `/admin` 에서 팀 4개·정원 15 확인.
2. `/admin` 로그인 → 팀 목록과 정원을 최종 확정한다. (정원은 나중에 늘릴 수는 있어도, 현재 인원보다 작게 줄일 수는 없다)
3. 오픈 시각을 지정하고 저장한다. `/` 에서 카운트다운이 뜨는지 확인한다.
4. 개발용 프로젝트에서 `npm run load-test` 를 한 번 더 돌려 전 항목 통과를 확인한다.
5. 참가자에게 배포 URL 과 "이름 + 학번(10자리)로 신청" 안내를 공지한다.

**오픈 직전 (T-10분)**

6. `/admin` 을 열어 둔다. 서버 시각이 화면 상단에 표시된다.
7. `/board` 를 프로젝터에 띄운다. 인증 없이 열람 가능하다.
8. `is_closed` 가 꺼져 있고 `opens_at` 이 정확한지 마지막으로 확인한다.

**오픈**

9. 별도 조작이 필요 없다. `opens_at` 이 지나는 순간 서버가 요청을 받기 시작한다.
10. 관리자 화면의 팀별 인원이 4초마다 갱신된다. 이상 징후는 감사 로그의 **"실패만 보기"** 로 확인한다.
...
11. **즉시 마감**을 켠다.
12. **CSV 다운로드** 로 최종 명단을 받는다. (`취소 포함` 버튼은 취소 이력까지 포함한 버전)
13. `/board` 화면을 캡처해 공지 채널에 올린다.
14. 이의 제기 대응이 필요하면 감사 로그로 시각·순번·사유를 그대로 제시할 수 있다.

---

## 8. 알려진 제약

- `lookup_registration()` 은 이름 + 학번을 아는 사람에게 배정 결과를 알려준다.
